...
impl HashHandler for BcryptHandler {
    async fn hash(&self, password: &str) -> Result<String> {
        ...
        task::spawn_blocking(move || hash(password, DEFAULT_COST).map_err(anyhow::Error::from))
            .await?
    }
    ...
    async fn verify(&self, password: &str, hash: &str) -> Result<bool> {
        ...
        task::spawn_blocking(move || verify(password, &hash).map_err(anyhow::Error::from)).await?
    }
}
