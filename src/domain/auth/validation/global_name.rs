...
pub fn validate_global_name(value: &str) -> Result<(), ValidationError> {
    ...
    if value.len() < 3 || value.len() > 30 {
        let e = ValidationError::new("invalid_length")
            .with_message("Global name must be between 3 and 30 characters.".into());
        return Err(e);
    }

    if !GLOBAL_NAME_REGEX.is_match(value) {
        let e = ValidationError::new("invalid_format")
            .with_message("Global name must contain only letters and numbers.".into());
        return Err(e);
    }
